...
import {
  Search, DollarSign, Users, Gauge,
  Car, Truck, Plane, Ship, Bike, Bus, Tractor, Filter, SortAsc, SortDesc
} from 'lucide-react';
import Alert from '@/components/ui/alert';
import FullscreenImage from '@/components/ui/fullscreenImage';

type SortOption = 'price_desc' | 'price_asc' | 'speed_asc' | 'speed_desc' | 'none';

function PageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const initialSort = searchParams.get('sort');
  const validSortOptions: SortOption[] = ['price_desc', 'price_asc', 'speed_asc', 'speed_desc', 'none'];
  ...
  const filteredAndSortedProperties = [...properties]
    ...
    .sort((a, b) => {
      switch (sortOption) {
        ...
        case 'speed_asc':
          return parseNumber(a.speed) - parseNumber(b.speed);
        case 'speed_desc':
          return parseNumber(b.speed) - parseNumber(a.speed);
        default:
          return 0;
      }
    });
  ...
  const getSortLabel = (option: SortOption) => {
    switch (option) {
      ...
      case 'speed_asc': return 'За зростанням швидкості';
      case 'speed_desc': return 'За спаданням швидкості';
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'Lightweight': return <Car className="h-4 w-4" strokeWidth={2} />;
      case 'Cargo': return <Truck className="h-4 w-4" strokeWidth={2} />;
      case 'Helicopters': return <Plane className="h-4 w-4" strokeWidth={2} />;
      case 'Boats': return <Ship className="h-4 w-4" strokeWidth={2} />;
      case 'Motorcycles': return <Bike className="h-4 w-4" strokeWidth={2} />;
      case 'Buses': return <Bus className="h-4 w-4" strokeWidth={2} />;
      case 'Tractors': return <Tractor className="h-4 w-4" strokeWidth={2} />;
          ...
          <div className="relative">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                {filterType === 'all' ? 'Тип транспорту' : filterType}
              </span>
            </button>
            {showTypeFilter && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-xl shadow-lg z-10 overflow-hidden border border-gray-200 dark:border-[#2a2c36]">
                {['all', 'Lightweight', 'Cargo', 'Helicopters', 'Boats', 'Motorcycles', 'Buses', 'Tractors'].map((type) => (
                  <button
                    key={type}
          ...
          <div className="relative">
            ...
            {showSortFilter && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-900 text-gray-900 dark:text-white rounded-xl shadow-lg z-10 overflow-hidden border border-gray-200 dark:border-[#2a2c36]">
                ...
                <button onClick={() => { setSortOption('speed_asc'); setShowSortFilter(false); }} className="flex items-center gap-2 w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700">
                  <SortAsc className="h-4 w-4" strokeWidth={2} /> За зростанням швидкості
                </button>
                <button onClick={() => { setSortOption('speed_desc'); setShowSortFilter(false); }} className="flex items-center gap-2 w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700">
                  <SortDesc className="h-4 w-4" strokeWidth={2} /> За спаданням швидкості
                </button>
              </div>
            )}
          </div>
