    ...
    <div className="w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 mt-8 sm:mt-10 lg:mt-12 mb-16 sm:mb-20 mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8">
        {Menu.map((item: MenuItem, idx: number) => (
          <Link key={idx} href={item.link ?? "#"} className="no-underline">
            <Card className="relative min-h-[120px] sm:min-h-[140px] p-4 sm:p-5 flex items-start gap-4 sm:gap-5 group border border-gray-200 dark:border-[#2a2c36] overflow-hidden">
              <div className="w-12 sm:w-14 h-12 sm:h-14 flex-shrink-0 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900 dark:to-blue-800 border border-gray-200 dark:border-[#2a2c36] flex items-center justify-center text-2xl sm:text-3xl shadow-sm">
              ...
              <CardContent className="flex-1 p-0 overflow-hidden">
                ...
                <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1 sm:mt-2 line-clamp-2 overflow-hidden text-ellipsis">
                  {item.description}
                </p>
              </CardContent>
              <div
                className="absolute right-3 sm:right-4 top-3 sm:top-4 inline-flex items-center justify-center rounded-full p-2 bg-gray-100 dark:bg-[#2a2c36] pointer-events-none"
                aria-hidden="true"
              >
                <svg
                  width="25"
